...
RSpec.describe(StillActive::AlternativesHelper) do
  describe(".leads_for") do
    ...
    before do
      allow(StillActive::RubygemsClient).to(receive(:info).with("pundit").and_return({"downloads" => 102}))
      allow(StillActive::RubygemsClient).to(receive(:info).with("cancancan").and_return({"downloads" => 90}))
      allow(StillActive::RubygemsClient).to(receive(:info).with("rabarber").and_return({"downloads" => 1}))
    end
    ...
    it("drops siblings whose download lookup fails, rather than the whole list") do
      allow(StillActive::RubygemsClient).to(receive(:info).with("cancancan").and_raise(StandardError))
      expect(described_class.leads_for(gem_name: "cancan", index: index))
        .to(eq(["pundit", "rabarber"]))
    end

    it("keeps zero-download siblings rather than dropping them") do
      allow(StillActive::RubygemsClient).to(receive(:info).with("rabarber").and_return({"downloads" => 0}))
      expect(described_class.leads_for(gem_name: "cancan", index: index))
        .to(eq(["pundit", "cancancan", "rabarber"]))
    end
    ...
    it("looks up at most MAX_SIBLINGS_CONSIDERED siblings for a huge category") do
      ...
      allow(StillActive::RubygemsClient).to(receive(:info).and_return({"downloads" => 1}))
      described_class.leads_for(gem_name: "x", index: {"x" => big_category})
      expect(StillActive::RubygemsClient).to(have_received(:info).at_most(described_class::MAX_SIBLINGS_CONSIDERED).times)
    end
  end
end
