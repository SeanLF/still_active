...
RSpec.configure do |config|
  ...
  config.before do
    ...
    # The deps.dev version prefetch batches before every fan-out. This answers only
    # its advisory canary (so the batch is trusted but holds nothing), leaving each
    # version to the per-version lookup the spec already stubs.
    canary = {"request" => {"versionKey" => {"system" => "PYPI", "name" => "django", "version" => "3.0.0"}}, "version" => {"advisoryKeys" => [{"id" => "GHSA-canary"}]}}
    stub_request(:post, "https://api.deps.dev/v3alpha/versionbatch").to_return(status: 200, body: {"responses" => [canary]}.to_json, headers: {"Content-Type" => "application/json"})
