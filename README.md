...
The activity (`--fail-if-critical`/`--fail-if-warning`) and vulnerability (`--fail-if-vulnerable`) gates apply to SBOM dependencies. **Poison-pill is scoped to flat-resolution ecosystems** (rubygems, pypi): there a dormant gem's cap forces the whole tree onto the old version. npm nests multiple versions and cargo lets majors coexist, so a transitive cap there pins a duplicate copy in one subtree rather than holding the tree hostage — and the one npm case that *does* block (a `peerDependencies` cap on a shared singleton) isn't exposed by the SBOM data source yet. Rather than over-claim, still_active suppresses poison for npm/cargo pending peer-dependency support. The other Ruby-shaped policy features also do not travel to the SBOM path yet: `.still_active.yml` suppressions and `--ignore` key on the gem name (SBOM results key on `ecosystem/name@version`), and `--fail-if-outdated` needs a libyear the cross-ecosystem lens doesn't compute.
