...
**When no service can say:** if none answers, or none knows the repository, the dependency is marked `repository_unavailable: true` rather than left with a blank `archived` that reads as "not archived". It emits no SA001 result, but a `status` that would claim clean (`ok`, `legacy`) reads `unknown`, the terminal and markdown show `?` for activity, `--fail-if-critical` and `--fail-if-warning` fail closed on it (an `activity` suppression accepts it), and the `--baseline` diff reports it as a regression. That includes a repository that is gone (deleted, or made private): a dependency whose source repository has disappeared isn't healthy either.
