...
**When no source answers:** if the advisory lookup fails (a timeout, a 5xx or 429 after one retry, a garbled or truncated answer) the dependency is marked `vulnerabilities_checked: false` rather than reported with zero advisories. It emits no SA003 result, since there is no advisory to point at, but its `status` reads `unknown` instead of `ok`, the terminal and markdown show `?` in place of `0`, and `--fail-if-vulnerable` fails closed on it with a warning naming it, the same stance as an unscored advisory. A 404 is an answer, not a failure: a private package deps.dev has never seen stays checked. On the native path a loaded ruby-advisory-db counts as an answer on its own.
