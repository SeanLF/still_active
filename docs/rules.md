...
**Where the answer comes from:** a GitHub repository is asked of GitHub's API and of ecosyste.ms, which mirrors the same fields: GitHub first with a token, ecosyste.ms first without one (GitHub allows 60 anonymous requests an hour), each the other's fallback when it can't answer (a rate limit longer than a minute's wait, an error status, a network failure). ecosyste.ms is asked only about a dependency from a public registry, and never after GitHub refused the token (401/403), so a private repository's name isn't sent to a third party; and "don't know" is never an answer from any service: ecosyste.ms's 404 for a repository it hasn't crawled, an answer with no `archived` field, and a forge's own 404 (which is also what a private repository looks like to anyone who can't see it) all move on to the next service. GitLab and Codeberg projects are asked of GitLab and Codeberg, on both audit paths. `repository_source` in the JSON says which service answered.

**When no service answers:** the dependency's `repository_check` says why, rather than leaving a blank `archived` that reads as "not archived". `"failed"` means a service couldn't answer (a rate limit, an error, a refused token): a rerun may help, so `--fail-if-critical` and `--fail-if-warning` fail closed on it (an `activity` suppression accepts it). `"unknowable"` means no service can say: there's no repository link, its host has no archived state (Google's go.googlesource.com, where `golang.org/x/*` lives), or every service asked didn't know it (ecosyste.ms hasn't crawled it, the forge 404s, which is how a private or deleted repository looks). A rerun won't change that, so it doesn't fail the gates. Either way it emits no SA001 result, a `status` that would claim clean (`ok`, `legacy`) reads `unknown`, and the terminal and markdown show `?` for activity; the `--baseline` diff reports a dependency that was answered and no longer is as a regression. The Go toolchain is `"not_applicable"`: it is the language, not a package with a repository.
...
**When no source answers:** if the advisory lookup fails (a timeout, a 5xx or 429 after one retry, a garbled or truncated answer) the dependency is marked `vulnerabilities_checked: false` rather than reported with zero advisories. A version record without its `advisoryKeys` list counts as no answer too (deps.dev sends `[]` when there are none, so a missing list is drift), as does an assessment that raised before reaching the lookup. It emits no SA003 result, since there is no advisory to point at, but a `status` that would claim clean (`ok`, or `legacy`) reads `unknown`, the `--baseline` diff treats it as a regression, the terminal and markdown show `?` in place of `0`, and `--fail-if-vulnerable` fails closed on it with a warning naming it, the same stance as an unscored advisory. A 404 is an answer, not a failure: a private package deps.dev has never seen stays checked. On the native path a loaded ruby-advisory-db counts as an answer on its own.

**SARIF level:** mapped from CVSS — `error` for ≥ 7.0, `warning` for 4.0–6.9, `note` below 4.0. An **unscored but confirmed** advisory is elevated to `warning` (never an informational note) and fails *closed* against a severity gate, since a confirmed advisory we can't score could be anything up to critical. Note that deps.dev stores only CVSS 3.x, so a **CVSS-4-only advisory** arrives with a `cvss3Score` of `0`; still_active treats a score of `0` as *unscored* (a real 0.0 never appears on a published advisory), so such an advisory reads as unscored rather than "low" — otherwise a HIGH finding could silently clear a gate or export as informational. **security-severity:** per-result, formatted CVSS3 (or CVSS2 fallback); omitted when unscored. A CVSS-4-only advisory's number is recovered from its v4 vector via [`cvss-suite`](https://rubygems.org/gems/cvss-suite) (the base score, matching what NVD and GHSA publish); the SARIF *level* floors at the OSV/GHSA label, so the number can sharpen a band upward but never lowers it. **CWE:** [CWE-1104](https://cwe.mitre.org/data/definitions/1104.html) (use of unmaintained third-party components, as a default — advisory-specific CWEs may also apply).
