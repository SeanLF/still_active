...
**When the repository can't be read:** GitHub's API is asked first, and when it can't answer (a rate limit longer than a minute's wait, an error status, a network failure) ecosyste.ms, which mirrors the same fields, is asked instead, but only for a dependency from a public registry, and never after a 401 or 403, so a private repository's name isn't sent to a third party. ecosyste.ms not knowing the repository (a 404, or no `archived` field) doesn't count as an answer there. If neither answers, or GitLab or Codeberg can't, the dependency is marked `repository_unavailable: true` rather than left with a blank `archived` that reads as "not archived". It emits no SA001 result, but a `status` that would claim clean (`ok`, `legacy`) reads `unknown`, the terminal and markdown show `?` for activity, `--fail-if-critical` and `--fail-if-warning` fail closed on it (an `activity` suppression accepts it), and the `--baseline` diff reports it as a regression. A repository the forge says doesn't exist (a 404) is an answer, not a failure.
