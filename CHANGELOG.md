...
- **A JSON Schema for the `--sbom` output**, `docs/still_active.sbom.schema.json`, which the output now names in `$schema`. Fields shared with the Gemfile audit reference its schema rather than copying it, and the suite validates real output against it. Writing it caught a gap: the Go toolchain's advisories carry `source: "osv"`, which the shared vulnerability definition didn't allow; it does now.
- **An on-disk cache of public sources' answers.** A rerun, or the baseline and current audits of one CI job, no longer ask the same questions twice: on a 300-component SBOM a second run made a handful of requests instead of hundreds and took half the time, with identical output. Each kind of answer is kept only as long as it holds (which advisories affect a version an hour, latest versions and advisory scores six hours, a published version's metadata a week; see `docs/configuration.md`), so a new advisory can surface up to an hour late, and `--no-cache` skips it. Nothing credentialed, no failure and no 404 is cached, and neither are the answers that must be current: OSV's check of which advisories really affect a version (it can drop a finding), the source-health canaries, and a malformed version record.
- **A source-health check before every audit, on both paths.** deps.dev is an alpha API and the advisory source for nearly everything, so still_active now checks once that it answers in shape (the advisory canary, which the Gemfile audit never ran before) and, per audited ecosystem, that its index is current: one of a small set of packages that publish often (`next`, `boto3`, `aws-partitions`, ...) must show a version published in the last 30 days. The sets were picked by replaying two years of deps.dev history, where the longest quiet stretch for any set is 10 days. Where either fails, deps.dev's advisories for that ecosystem count as unchecked (so `--fail-if-vulnerable` fails closed on them), a warning says why, and both JSON outputs carry a `source_health` block. ruby-advisory-db and the Go toolchain's OSV lookup still answer on their own. It costs one request plus about one per ecosystem. It can't catch a single stale package in a live index.
...
- **Without a GitHub token, a repository ecosyste.ms hasn't crawled no longer reads as "not archived".** ecosyste.ms answers first in that mode, and about one repository in twenty isn't in it; that "don't know" used to read as "not archived". GitHub is now asked about those, without a token (public repositories only, 60 requests an hour), and one neither service can answer is marked `repository_unavailable`. Private-source repositories are no longer sent to ecosyste.ms, with or without a token.
- **A rate-limited run no longer reads an archived repository as active.** When GitHub's API refused a repository lookup (a burst of concurrent requests trips its secondary rate limit, even with a token), `archived` came back blank, and a blank `archived` read as "not archived": in a real run, two archived repositories came out `ok` and `stale`. GitHub failures now fall back to ecosyste.ms, which mirrors the same fields (only for dependencies from a public registry, and not after a 401/403, so a private repository's name never goes to a third party), so a rate limit alone no longer loses the signal (verified on a 300-component SBOM with every GitHub call refused: the same repositories read archived). When no source can answer, or the dependency's assessment failed part-way, it carries `repository_unavailable: true`: an `ok` or `legacy` status reads `unknown`, the terminal and markdown show `?`, both JSON summaries count `repositories_unavailable`, the enriched CycloneDX marks it, the `--baseline` diff calls it a regression, and **`--fail-if-critical` / `--fail-if-warning` fail closed on it**, with a warning naming it (an `activity` suppression accepts it). A repository the forge says doesn't exist is an answer, not a failure.
- **The Go toolchain (`pkg:golang/stdlib`) is assessed from endoflife.date and OSV, not deps.dev.** deps.dev keys it `go1.27.1` where the purl says `1.27.1`, its index stops at go1.25.5, and its advisory list is the same for every version, so stdlib read as behind a stale "latest" and **every stdlib pin reported zero advisories**. It now reads its latest release from endoflife.date, its advisories from OSV for that exact version, and an end-of-life release line as deprecated, naming the line to move to. If OSV can't answer, its advisories are marked unchecked (see below) and a warning says so.
