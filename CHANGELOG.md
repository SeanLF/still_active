...
- **SARIF `tool.driver.version` is now populated.** The tool component set `semanticVersion` but left `version` null, so a consumer reading `tool.driver.version` (rather than `semanticVersion`) saw nothing. Both fields now carry `StillActive::VERSION`.
