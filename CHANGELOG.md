...
- `--fail-if-vulnerable=<severity>` no longer silently passes a confirmed advisory that carries no CVSS score. Previously an unscored advisory made the severity comparison read as "below threshold", so a real finding cleared the gate, worst of all for a freshly disclosed CVE (scoring lags disclosure). It now **fails closed**: an unscored advisory that isn't explicitly suppressed trips the threshold gate, with a per-gem stderr note explaining why so it can be reviewed and fixed or accepted via a `.still_active.yml` suppression. Bare `--fail-if-vulnerable` was already correct (it fails on any advisory).
- The native (Bundler) path no longer drops a confirmed advisory when its deps.dev detail fetch fails (429/timeout/5xx). It previously `filter_map`'d the failed lookup away, which could zero out `vulnerability_count` and read a known-vulnerable gem as clean, clearing every vulnerability gate. It now keeps a minimal advisory for the un-enriched key (mirroring the cross-ecosystem lens; ruby-advisory-db fills the score on merge when it also carries the advisory), so the finding survives and the fail-closed severity logic applies. Together these close the fail-open on both the native and `--sbom` paths.
